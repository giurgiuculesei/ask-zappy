...
export default function Topics({
    ...
    const onChange = (newLevel: Level) => {
        ...
        window.history.replaceState(
            null,
            "",
            `/ib-math/analysis-and-approaches/question-bank/${encodeURIComponent(newLevel)}`
        );
    };
                        ...
                        <div className="inline-flex items-center gap-2 rounded-2xl bg-slate-100 p-2">
                            {(['sl', 'hl'] as Level[]).map((opt) => (
                                <label className="cursor-pointer" key={opt}>
                                    <input type="radio" name="level" value={opt} className="peer sr-only" defaultChecked={opt === level} onChange={() => onChange(opt)} />
